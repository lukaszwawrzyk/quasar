...
object AtomicRef {
  ...
  final class Ops[V, S[a] <: ACopK[a]](implicit S: AtomicRef[V, ?] :<<: S)
  ...
  object Ops {
    implicit def apply[V, S[a] <: ACopK[a]](implicit S: AtomicRef[V, ?] :<<: S): Ops[V, S] =
      new Ops[V, S]
  }
  ...
  object onSet {
    ...
    final class Aux[V] {
      ...
      def apply[S[a] <: ACopK[a], F[_]: Applicative]
          (f: V => F[Unit])
          (implicit
            S0: F :<<: S,
            S1: Ref :<<: S
  ...
  object zoom {
    ...
    final class Aux[A, B: Equal](lens: Lens[A, B]) {
      def into[S[a] <: ACopK[a]](implicit S: AtomicRef[A, ?] :<<: S)
          : AtomicRef[B, ?] ~> Free[S, ?] = {
