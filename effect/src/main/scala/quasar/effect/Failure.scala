...
object Failure {
  ...
  final class Ops[E, S[a] <: ACopK[a]](implicit S: Failure[E, ?] :<<: S)
  ...
  object Ops {
    implicit def apply[E, S[a] <: ACopK[a]](implicit S: Failure[E, ?] :<<: S): Ops[E, S] =
      new Ops[E, S]
  }
  ...
  def attempt[S[a] <: ACopK[a], E](implicit S: Failure[E, ?] :<<: S): S ~> EitherT[Free[S, ?], E, ?] = new (S ~> EitherT[Free[S, ?], E, ?]) {
    def apply[A](sa: S[A]) =
      S.prj(sa) match {
  ...
  def monadError_[E, S[a] <: ACopK[a]](implicit O: Ops[E, S]): MonadError_[Free[S, ?], E] =
