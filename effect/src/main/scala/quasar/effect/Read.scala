...
object Read {
  ...
  final class Ops[R, S[a] <: ACopK[a]](implicit S: Read[R, ?] :<<: S)
    ...
    def local(f: R => R): FreeS ~> FreeS = {
      val g: Read[R, ?] ~> FreeS = injectFTCopK compose contramapR(f)
  ...
  object Ops {
    implicit def apply[R, S[a] <: ACopK[a]](implicit S: Read[R, ?] :<<: S): Ops[R, S] =
      new Ops[R, S]
  }
  ...
  def monadReader_[R, S[a] <: ACopK[a]](implicit O: Ops[R, S]): MonadReader_[Free[S, ?], R] =
