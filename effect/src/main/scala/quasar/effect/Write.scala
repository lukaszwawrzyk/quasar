...
import quasar.fp.{:<<:, ACopK, TaskRef}
...
object Write {
  ...
  final class Ops[W, S[a] <: ACopK[a]](implicit S: Write[W, ?] :<<: S) extends LiftedOps[Write[W, ?], S] {
    def tell(w: W): FreeS[Unit] = lift(Tell(w))
  }

  object Ops {
    implicit def apply[W, S[a] <: ACopK[a]](implicit S: Write[W, ?] :<<: S): Ops[W, S] = new Ops[W, S]
  }
