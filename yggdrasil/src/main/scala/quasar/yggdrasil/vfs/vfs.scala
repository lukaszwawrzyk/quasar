...
import quasar.fp.{:<<:, ACopK, TwoElemCopKOps}
  ...
  def initVersion[S[a] <: ACopK[a], A: Equal: Show](
    ...
    def checkAndUpdateVersion: Free[S, Unit] =
      for {
        ...
        verBV <- verStream.runFoldMap(ι)
          .mapSuspension(CopK.NaturalTransformation.of[PosixWithTaskCopK, S](injectNTCopK[POSIXOp, S], injectNTCopK[Task, S]))
        ver <- lift(
...
object SerialVFS {
  private[SerialVFS] type S[A] = CopK[POSIXOp ::: Task ::: TNilK, A]
  ...
  def apply(root: File): Stream[Task, SerialVFS] = {
    for {
      ...
      interp = λ[S ~> Task](_.toDisjunction.fold(pint, NaturalTransformation.refl[Task]))
