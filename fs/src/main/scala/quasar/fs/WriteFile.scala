...
object WriteFile {
  ...
  object Ops {
    implicit def apply[S[a] <: ACopK[a]](implicit U: Unsafe[S]): Ops[S] =
      new Ops[S]
  }
  ...
  /** Low-level, unsafe operations. Clients are responsible for resource-safety
    * when using these.
    */
  final class Unsafe[S[a] <: ACopK[a]](implicit S: WriteFile :<<: S)
  ...
  object Unsafe {
    implicit def apply[S[a] <: ACopK[a]](implicit S: WriteFile :<<: S): Unsafe[S] =
      new Unsafe[S]
  }
