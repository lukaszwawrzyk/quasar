    ...
    new Delay[RenderQScriptDSL, MapFunc[T, ?]] {
      ...
      def apply[A](fa: RenderQScriptDSL[A]) = {
        (base: String, mf: MapFunc[T, A]) =>
          val MapFuncCore = CopK.Inject[MapFuncCore[T, ?], MapFunc[T, ?]]
          val MapFuncDerived = CopK.Inject[MapFuncDerived[T, ?], MapFunc[T, ?]]

          val (label, children) = mf match {
            case MapFuncCore(value) => value match {
            ...
            case MapFuncDerived(value) => value match {
